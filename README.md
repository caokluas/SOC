...
| Respond | 自动化工具箱 | **有**（部分） |
| Respond | 协同作战 IM 拉群 | **有（占位）** war-rooms；**KN 六 Bot 出站已通** |
| Recover | 灾备演练 RTO/RPO | **有（占位）** |
| Recover | AI Memory | **有（占位）** |
| Recover | 复盘报告 | **有** |

**统计**：已实现演示能力 **10** + 无外部依赖占位 **8**。真下发 / 真备份 / 强制 LLM 仍依赖外部；**KN Chat 分类型告警出站已本地验证**（生产群与真源见 §4.6 / roadmap §14）。
...
| **v0.5.x** | 进行中 | KN 六类 Bot 分路由出站 + code_audit 闭环（§4.6） |
| **v0.6.0** | 规划 | **真联动首发**：一类真实数据源持续输入 + 生产 IM 群 + 真 ITSM |
| **v0.7.0** | 规划 | 生产门禁过半：SSO/RBAC + 审计 |
| **v1.0.0** | 规划 | 共享环境可上线：真数据 + 真动作 + 门禁闭环 |

### 4.6 KN Chat 六类告警与 IM 闭环（摘要）

> 出站：`kn-chat-data-flow.md` · 路由：`kn-chat-bot-routing.md` · 闭环：`im-closed-loop-framework.md` · 架构分层：`architecture.md` §6.1 · 路径：roadmap **§14 / §15**。
...
回归：`tools/kn-allbots-verify.ps1` · 样本：`docs/samples/kn-allbots-risk-inputs.json`。

### 4.7 下一步实施顺序（建议）

1. **KN 生产化（CL-6）**：六 Bot 绑定告警群 chat_id；Secret 进部署系统（勿入库）  
...
**需外部前置才从「占位」变「真」的能力**（节选）：SSO/RBAC · 真实日志 Topic · 真零信任/云写动作 · 真 Jira · **生产 KN 告警群与持续真源** · 真资产图谱 · 真 DLP/设备 API · 真 TI Feed · Memory 向量化 · RTO/RPO 备份联动。  
...
| KN 六类出站 | 六路 `kn_chat_sent:{route}`；详情可读；不串台 |
...
细则：`docs/test-plan.md` · `docs/evaluation.md` · `docs/kn-chat-data-flow.md`。
...
| IM / KN | 六 Bot 出站 + 入站 poller；`im/actions`；`SourceSyncAdapter` |
...
# 确保 KN_CHAT_* / CODE_AUDIT_PORTAL_* / KN_CHAT_INBOUND_ENABLED 已在环境中
...
| `docs/kn-chat-data-flow.md` / `kn-chat-bot-routing.md` | 出站流转与路由命名 |
