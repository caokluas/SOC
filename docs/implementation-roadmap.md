...
| 通知联动 | 本地台账 + **IM 六类 Bot 分路由** + 可选通用 Webhook | 高风险按类型进对应机器人；详情可读 | **IM 出站已验证**；待生产群/真源输入（§14） |
...
| Respond | 协同作战 IM 自动拉群 | **有（占位）** war-rooms；**出站告警已接 IM 六 Bot**（§14） |
...
| 真 Jira / IM | JIRA_*；`IM_*` 已通六 Bot 私聊 Demo | **IM 分路由出站已验证**；待生产告警群、Jira 真 API、war-room 真拉群 |
...
| P0 | IM / Jira 真协同 | **IM 六 Bot 出站已通**；下一步生产群 chat_id + 真源输入（§14）+ Jira 凭证 |
...
| 最大缺口集群 | §9 真外部依赖（设备下发 / **生产 IM 群与真源** / SSO / 真 SOAR）；IM 六 Bot **出站 Demo 已通**见 §14 |
...
> 权威细则：`im-data-flow.md`（流转/效果/依赖）· `im-bot-routing.md`（命名/环境变量）。
...
见 `im-data-flow.md` §6（样例回归 → code_audit 真仓 → access → data → cloud → TI → soc_ops 值班群 → 门禁）。

回归：`tools/im-allbots-verify.ps1`。
...
`CL-1 深链` → `CL-2 im/actions API` → `CL-3/4 IM 按钮与命令` → `CL-5 Ticket/Portal 同步` → `CL-6 生产群与降噪`  
...
> 在 IM/code_audit 通用闭环之上，优先支撑 **四类专项材料录入与聚合**；真源对接可后续增量接入。
