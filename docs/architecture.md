...
| 自动化处置中心 | 派单、通知、Playbook、例外提醒、月报；IM 六 Bot 出站；IM 回流状态机 | Ticket、Notification、PlaybookExecution、IMAction、MonthlyReport |
| IM 通道与源反哺 | 分路由 sendMessage / getUpdates；S8 `SourceSyncAdapter` | im_*、source_sync_* |
...
```text
通道层                核心层                 编排层              适配层
IM 出站     →   ApplyIMAction    →   afterIMAction  →  SourceSyncAdapter
im_inbound      （Incident 状态机）      ├─ Ticket/ITSM       ├─ code_audit → Portal
...
| 层次 | 代码落点 | 允许依赖 | 禁止 |
|---|---|---|---|
| 通道层 | `im_channel.go`、`im_inbound.go` | Bot Token、Telegram 兼容 API | 写 Portal URL / 改 Finding 生命周期 |
...
flowchart LR
  ...
  DET --> OUT["S3 IM sendMessage + 深链 + 按钮"]
  OUT --> IM["S4 人点按钮/命令"]
...
环境变量边界：`IM_*`（通道）· `ITSM_WEBHOOK_URL`（工单）· `CODE_AUDIT_PORTAL_*` / `SOURCE_SYNC_WEBHOOK_*`（适配层）。
...
SOAR 自动化与人机协同。能力重心：Playbook 编排、动作工具箱、作战室协同、**IM 六 Bot 告警出站与回流**。依赖：设备写权限、响应授权流程、IM Bot Token。数据：告警事件、资产归属、工单、IM 动作台账（`IMAction`）、源系统回写结果。
...
| 方向 | 建议 |
|---|---|
| 真实 ITSM / IM | Jira、ServiceNow；IM 生产告警群（CL-6）；飞书/Teams 可作第二通道 |
